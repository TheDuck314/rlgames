...
def optionally_restore_from_checkpoint(sess, saver, ckpt_dir):
    while True:
        response = raw_input("Restore from checkpoint in {} [y/n]? ".format(ckpt_dir)).lower()
        if response == 'y': 
            return restore_from_checkpoint(sess, saver, ckpt_dir)
        if response == 'n':
            return 0
